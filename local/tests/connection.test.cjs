...
const source = fs.readFileSync(require('node:path').join(__dirname, '../connection.js'),'utf8');
function harness(get = () => new Promise(() => {})) {
 ...
 document:{createElement(){return {querySelector(){return label},querySelectorAll(){return buttons}}},getElementById(){return {prepend(){}}},addEventListener(n,f){events[n]=f}},
