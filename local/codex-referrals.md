...
`shared/core.js` uses this routing when ordered examinations and CPT codes match the appointment. Independent parenthesized items in the services field (such as the trailing `(SHA, ...)` request memo) are excluded from service comparison and regex classification; a note directly following a DBQ/Questions examination label is also excluded. Parentheses belonging to an examination name, such as `SYPHILIS TEST(VDRL, RPR, ART)` or `ECHOCARDIO(2D)`, remain part of the examination and changes inside them invalidate the imported classification. Parsing handles commas inside parentheses, nested parentheses and full-width parentheses. The original text remains stored and displayed. Changes to examinations outside parentheses or CPT codes still invalidate the routing. The existing regex handles records without matching routing. Unknown clinic assignments produce a review notice. Saved referral fields continue to take precedence over generated purposes. `local/script.js` no longer overwrites these purposes a second time after opening the modal.
