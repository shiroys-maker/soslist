...
tableBody.addEventListener('click', (e) => {
    ...
    if (target.classList.contains('show-toggle-cell')) {
        ...
        docRef.get().then(doc => {
            if (doc.exists) {
                docRef.update({ isShown: !doc.data().isShown });
            }
        });
        return;
    }
...
activityEvents.forEach(eventName => {
    window.addEventListener(eventName, startLogoutTimer, true);
});
    ...
    unsubscribe = db.collection("appointments")
      ...
      .onSnapshot(querySnapshot => {
          ...
          appointments.forEach(appointment => {
              ...
              const displayServicesText = (data.services || []).join(', ');
...
function handleViewPdf(docId) {
    db.collection('appointments').doc(docId).get().then(doc => {
        ...
        storage.ref(fileName).getDownloadURL()
            ...
            .catch(error => alert('PDFの表示中にエラーが発生しました。'));
    });
}
...
function openDetailsModal(docId) {
  db.collection('appointments').doc(docId).get().then(doc => {
    ...
    detailsHTML += `<div class="detail-row"><div class="detail-item"><strong>予約日時</strong><span>${displayDate.replace('<br>',' ')}</span></div><div class="detail-item"><strong>契約番号</strong><span>${data.contractNumber || ''}</span></div></div>`;
    detailsHTML += `<div class="detail-row"><div class="detail-item"><strong>氏名</strong><span>${data.claimantName || ''}</span></div><div class="detail-item"><strong>生年月日(年齢)</strong><span>${data.dateOfBirth || ''} (${displayAge})</span></div></div>`;
    detailsHTML += `<div class="detail-row"><div class="detail-item"><strong>電話番号</strong><span>${data.japanCellPhone || ''}</span></div><div class="detail-item"><strong>検査内容</strong><span>${(data.services || []).join(', ')}</span></div></div>`;
    detailsHTML += `<div class="detail-row"><div class="detail-item"><strong>来</strong><span>${data.isShown ? '✅' : ' '}</span></div><div class="detail-item"></div></div>`;
...
function saveNotes() {
    ...
    db.collection('appointments').doc(docId).update({
        ...
        alert('メモを保存しました。');
        closeDetailsModal();
    }).catch(error => alert('メモの保存に失敗しました。'));
}
...
function printInvoice() {
    ...
    if (!fromDateStr || !toDateStr) return alert('FromとToの両方の日付を選択してください。');
      ...
      .then(querySnapshot => {
          ...
          querySnapshot.forEach(doc => {
              ...
              if (data.examinationFee) {
                  ...
                  if (!isNaN(parsedFee)) feeForSum = parsedFee;
              } else {
                  const cptCodeString = (data.cptCode || []).join(', ').replace(/\s/g, '');
                  if (cptCodeString === "92557,92550,VA0004") feeForSum = 220000;
              }
              totalFee += feeForSum;
          });
          if (records.length === 0) return alert('選択された期間に、SHOWがチェックされたレコードはありませんでした。');
...
function generateInvoiceHTML(records, from, to, total) {
    ...
    records.forEach(data => {
        ...
        if (!displayFee) {
            const cptCodeString = (data.cptCode || []).join(', ').replace(/\s/g, '');
            if (cptCodeString === "92557,92550,VA0004") displayFee = '220,000';
        }
        if (!displayFee) displayFee = '0';
        tableRows += `<tr><td>${displayDate}</td><td>${data.contractNumber || ''}</td><td>${data.claimantName || ''}</td><td>${data.dateOfBirth || ''}</td><td>${(data.cptCode || []).join(', ')}</td><td class="fee-cell">${displayFee}</td></tr>`;
    });
    const invoiceHTML = `<!DOCTYPE html><html lang="ja"><head><meta charset="UTF-8"><title>Invoice</title><style>body { font-family: sans-serif; } .invoice-container { max-width: 800px; margin: auto; padding: 20px; } h1, h2 { text-align: center; } table { width: 100%; border-collapse: collapse; margin-top: 20px; } th, td { border: 1px solid #ddd; padding: 8px; text-align: left; font-size: 12px; } th { background-color: #f2f2f2; } tfoot td { font-weight: bold; font-size: 14px; } .fee-cell { text-align: right; } @media print { .no-print { display: none; } }</style></head><body><div class="invoice-container"><h1>INVOICE</h1><h2>Period: ${from} to ${to}</h2><table><thead><tr><th>予約日時</th><th>契約番号</th><th>氏名</th><th>生年月日</th><th>CPTCODE</th><th class="fee-cell">検査費</th></tr></thead><tbody>${tableRows}</tbody><tfoot><tr><td colspan="5" style="text-align: right;">合計:</td><td class="fee-cell">${total.toLocaleString()}</td></tr></tfoot></table><br><button class="no-print" onclick="window.print()">このページを印刷</button></div></body></html>`;
...
function saveServices() {
    ...
    }).then(() => closeServicesEditModal())
      .catch(error => alert('検査内容の更新に失敗しました。'));
}
